...
# -----------------------------------------------------------------------------
# 4. REPORTLAB PDF & GAUGES (SINGLE LINE BRACKET SAFE)
# -----------------------------------------------------------------------------
def create_circular_trust_gauge(score: float, title: str, is_trust: bool = True) -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [{'range': [0, 35], 'color': 'rgba(239, 68, 68, 0.1)'}, {'range': [35, 70], 'color': 'rgba(245, 158, 11, 0.1)'}, {'range': [70, 100], 'color': 'rgba(56, 189, 248, 0.1)'}],
        }
    ))
...
def generate_pdf_report(analysis: dict) -> bytes:
    ...
    story = [Paragraph("FraudShield AI — Security Audit Report", title_style), Paragraph(sub_text, subtitle_style), HRFlowable(width="100%", thickness=1.5, color=colors.HexColor('#0284C7'), spaceAfter=15), Paragraph("Threat Assessment Summary", section_heading)]

    summary_data = [["Target URL", str(analysis['url'])], ["Threat Level", str(analysis['threat_level'])], ["Trust Score", f"{analysis['trust_score']} / 100"], ["Fraud Risk Probability", f"{round(analysis['fraud_probability'] * 100, 1)}%"], ["AI Model Confidence", f"{analysis['confidence_score']}%"], ["Threat Category", str(analysis['threat_category'])]]
    
    t = Table(summary_data, colWidths=[150, 370])
    style_cmds = [('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#F8FAFC')), ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor('#E2E8F0')), ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'), ('TEXTCOLOR', (0, 0), (-1, -1), colors.HexColor('#0F172A')), ('PADDING', (0, 0), (-1, -1), 6)]
    t.setStyle(TableStyle(style_cmds))
    
    story.append(t)
    story.append(Spacer(1, 12))

    story.append(Paragraph("Category-Sp